...
Vagrant.configure(2) do |config|
  ...
  config.vm.synced_folder ".", "/scribe3" ,
                          :owner => 'vagrant',
                          :group => 'vagrant',
  ...
  # documentation for more information about their specific syntax and use.
   config.vm.provision "shell", inline: <<-SHELL
	  sudo sh /scribe3/scripts/install_dev_environment.sh /scribe3
   SHELL

  # uncomment the following line to use the vagrant-config.sh as provisioning script
  #config.vm.provision "shell", path: "scripts/install_dev_environment.sh"
end
